<div class="modal fade modal-primary" id="carModelModal" tabindex="-1" role="dialog" aria-labelledby="carModelLabel" aria-hidden="true">
    <div class="modal-dialog">
        <div class="modal-content rounded-0">
            <div class="modal-header border-bottom mt-0 p-1 pl-4 mb-0">
              <h5 class="mt-0 pt-2 pb-0 mb-1" style="color:#a5a5a5;">Add Car Model</h5>
            </div>
            <div class="modal-body">
              <form method="POST" action="{{ route('carModels.store') }}" aria-label="{{ __('Register') }}" enctype='multipart/form-data'>
                  @csrf
                  <div class="form-group row p-0">
                      <!--name-->
                      <div class="col-12 col-md-6">
                          <input id="model_name" name="model_name" value="Name" onclick="if(this.value =='Name')this.value=''" type="text" class="text-capitalize rounded-0 no-outline form-control{{ $errors->has('model_name') ? ' is-invalid' : '' }}" value="{{ old('model_name') }}" placeholder="Name" required style="border: 1px solid #a5a5a5;">

                          @if ($errors->has('model_name'))
                              <span class="invalid-feedback" role="alert">
                                  <strong>{{ $errors->first('model_name') }}</strong>
                              </span>
                          @endif
                      </div>

                      <!--maker-->
                      <div class="col-12 col-md-6">
                        <select id="car_maker" name="car_maker" class="form-control no-outline rounded-0" style="border: 1px solid #a5a5a5;">
                          <option value="0" selected hidden disabled>Car Maker</option>
                          @foreach((new App\Models\Product\CarMaker())->getAllMakers() as $carmaker)
                            <option value="{{$carmaker->id}}">{{$carmaker->name}}</option>
                          @endforeach
                        </select>
                      </div>
                  </div>

                  <div class="form-group row p-0">
                      <!--from-->
                      <div class="col-12 col-md-4">
                        <select id="from" name="from" style="border: 1px solid #a5a5a5;" class="form-control no-outline rounded-0">
                          <option value="0" selected hidden disabled>From</option>
                          @for($i=1980; $i<'2020'; $i++)
                            <option value="{{$i}}">{{$i}}</option>
                          @endfor
                        </select>
                      </div>

                      <!--to year-->
                      <div class="col-12 col-md-4">
                        <select id="to" name="to" style="border: 1px solid #a5a5a5;" class="form-control no-outline rounded-0">
                          <option value="0" selected hidden disabled>To</option>
                          @for($i=1980; $i<'2020'; $i++)
                            <option value="{{$i}}">{{$i}}</option>
                          @endfor
                        </select>
                      </div>

                      <!--image-->
                      <div class="col-12 col-md-4">
                          <button type="button" class="pt-1 pb-1 btn btn-primary no-outline rounded-0 w-100" onclick="uploadCarModelImage()" style="font-size:16px;"><span class="fa fa-plus mr-2"></span>Image</button>
                          <input id="car_modal_image" name="car_modal_image" class="d-none" type="file"/>
                      </div>
                  </div>

                  <!--details-->
                  <div class="form-group row">
                      <div class="col-12">
                        {{Form::textarea('model_detail',"Details..." ,['onclick' => "if(this.value =='Details...')this.value=''", 'style' => "height:120px;border: 1px solid #a5a5a5;", 'class' => "rounded-0 no-outline form-control", 'placeholder' => "Details..."])}}

                          @if ($errors->has('model_detail'))
                              <span class="invalid-feedback" role="alert">
                                  <strong>{{ $errors->first('model_detail') }}</strong>
                              </span>
                          @endif
                      </div>
                  </div>

                  <!--submit button-->
                  <div class="form-group row mb-0">
                      <div class="col-6">
                          <button class="no-outline rounded-0 btn btn-primary pt-1 pb-1" type="submit">
                              {{ __('Add Model') }}
                          </button>
                      </div>

                      <div class="col-6">
                          <button class="float-right no-outline rounded-0 btn btn-primary pt-1 pb-1" type="button" data-dismiss="modal" aria-label="Close" data-toggle="modal" data-target="#carMakerModal">
                              {{ __('Add Maker') }}
                          </button>
                      </div>
                  </div>
              </form>
            </div>
        </div>
    </div>
</div>
