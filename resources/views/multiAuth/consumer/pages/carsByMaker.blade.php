                        ...
                        @foreach($carModel->getCars() as $car)
                          <div class="col-3 border" style="padding:15px;position:relative;height:200px;">
                            <a class="nav nav-link m-0 p-0" href="{{ route('find.car.details', $car->id) }}">
                              <h5 class="text-capitalize" style="font-size:16px;color:rgba(33,37,41,0.8);font-weight:normal;">{{$car->name}}</h5>
                              <img class="float-right" src="{{url($car->getImage())}}" data-bs-hover-animate="pulse" style="width:60%;position:absolute;bottom:10px;right:10px;">
                            </a>
                          </div>
                        @endforeach
                        <!-- coming soon -->
